...
def _card_type_order(prefix=''):
    ...
    return Case(
        ...
        When(**{f: 'creature'}, then=Value(5)),
        When(**{f: 'device'}, then=Value(6)),
        When(**{f: 'effect'}, then=Value(7)),
        When(**{f: 'epic_event'}, then=Value(8)),
        When(**{f: 'used_interrupt'}, then=Value(9)),
        When(**{f: 'lost_interrupt'}, then=Value(10)),
        When(**{f: 'jedi_test'}, then=Value(11)),
        When(**{f: 'location'}, then=Value(12)),
        When(**{f: 'objective'}, then=Value(13)),
        When(**{f: 'starship'}, then=Value(14)),
        When(**{f: 'vehicle'}, then=Value(15)),
        When(**{f: 'weapon'}, then=Value(16)),
        default=Value(17),
